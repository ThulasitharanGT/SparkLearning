...
import org.apache.spark.sql.functions.{col, current_timestamp, from_json, length, lit}
...
import org.controller.persistingInsideJob.jobHelper.{argsToMapConvert, expiryCheck, getExistingRecords, idGetterTeam, idListToStringManipulator}
...
object combinedResolvingStateAlone {
  def main(args: Array[String]): Unit = {
      ...
      (batchDF: DataFrame, batchID: Long) => {
        ...
        Try {
          ...
          case true =>
            ...
            existingDriverRecInState = driverPersistDFOut.drop("receivedTimeStamp", "receivedBatch")
    ...
    */
            ...
            val releasedDriver = driverStateAndValid.where("released_in.teamId is not null").selectExpr("state_out.*")
    ...
    driverStreamDF.writeStream.format("console").outputMode("update").option("checkpointLocation", inputMap("driverStreamCheckpoint")).foreachBatch(
      (batchDF: DataFrame, batchID: Long) => {
        ...
        Try {
          ...
          case true =>
            existingTeamRecInState = teamPersistDFOut.drop("receivedTimeStamp", "receivedBatch")
        }
        ...
        //    val retainedTeamDF = teamPersistDFOut.except(releasedTeamDF)
            ...
            val releasedTeamBatchDF = batchAndTeamState.where("released_in.teamId is not null").selectExpr("state_out.*")


            teamPersistDFOut = retainedTeamDF
            releasedTeamBatchDF.persist(StorageLevel.MEMORY_AND_DISK_SER)
            releasedTeamBatchDF.drop("receivedTimeStamp", "receivedBatch").withColumn("releasedTeamInDriverState", lit("releasedTeamInDriverState")).show(false)
