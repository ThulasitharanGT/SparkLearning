...
import org.util.readWriteUtil.{checkPointLocationCleaner, deltaTableRead, readDF, readStreamFunction, writeDF}
...
//  val dateFormat=new SimpleDateFormat("YYYY-MM-DD HH:mm:ss.SSS")
  ...
  //  import spark.implicits._
    ...
    val persistAggPath= inputMap("persistAggPath")
 ...
 /*   val checkpointExists = s"hdfs dfs -ls ${checkpointLocation}" !; // run time error might occur if you fail to terminate the statement
    ...
    readStreamDF.writeStream.format("console").outputMode("append").option("checkpointLocation",checkpointLocation).foreachBatch(
      (microBatchDF:org.apache.spark.sql.DataFrame,batchID:Long) =>
        {
          finalTableManipulator(persistPath,inputPath,persistAggPath,inputMap)
        }
    ).start
 ...
 val tmpColNames=Seq("driverName","points","receivedTimeStamp","lastUpdatedTimeStamp")
  ...
  def finalTableManipulator(persistPath:String,inputPath:String,persistAggPath:String,inputMap:collection.mutable.Map[String,String])={
   ...
   // val deltaTableFinal=DeltaTable.forPath(spark,persistPath)
    /*
    val inputPath="hdfs://localhost:8020/user/raptor/hadoop/deltaLake/tmp2/"
    val persistAggPath=""
    val persistPath="hdfs://localhost:8020/user/raptor/hadoop/deltaLake/tmpFinal"

    */
    inputMap.put(projectConstants.filePathArgValue,persistPath)
   val deltaTableFinal=deltaTableRead(spark,inputMap)
    inputMap.put(projectConstants.fileTypeArgConstant,projectConstants.deltaFormat)
    inputMap.put(projectConstants.filePathArgValue,persistPath)
    val deltaTableFinalDF=readDF(spark,inputMap)
    // val deltaTableFinalDF=spark.read.format(projectConstants.deltaFormat).load(persistPath)
    inputMap.put(projectConstants.filePathArgValue,inputPath)
    val deltaTableBatch=deltaTableRead(spark,inputMap).toDF.groupBy("driverName").agg(sum("points").as("points"),max(col("receivedTimeStamp").cast(TimestampType)).as("receivedTimeStamp"))

   // val deltaTableBatch=DeltaTable.forPath(spark,inputPath).toDF.groupBy("driverName").agg(sum("points").as("points"),max(col("receivedTimeStamp").cast(TimestampType)).as("receivedTimeStamp"))
 ...
 //   val deltaTableBatch=deltaTableBatchNameAndPoints.join(deltaTableBatchLastInTime,Seq("driverName"))
    ...
    reComputeDriversPositions(spark,persistPath,persistAggPath,inputMap) // intermediate layer is input for this
  }
