...
object fixBronzeToSilver extends SparkOpener {
  ...
  def main(args:Array[String]):Unit={
    ...
    deltaTableBronze.toDF.where("brand not in ('Datsun')").drop("number_of_owners").withColumn("number_of_owners",when(col("brand") === "Lambhorghini",4).when(col("brand") === "Konegzegg",3).when(col("brand") === "Pagani",2).when(col("brand") === "Bugatti",1).when(col("brand") === "Suzuki",10).when(col("brand") === "Ford",5).otherwise(15)  ).write.mode(modeForDeltaWrite).option("spark.sql.sources.partitionOverwriteMode","dynamic").format("delta").partitionBy ("brand", "model", "year", "month").save(outputBasePath+deltaTableBaseName+"_Silver")
    println("---------------------------------------SILVER--------------------------------------------------")
